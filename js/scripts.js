...
let workAddressBook = new AddressBook();
...
function listContacts(addressBookToDisplay) {
  ...
  Object.keys(addressBookToDisplay.contacts).forEach(function(key) {
    ...
    li.append("," + " " + contact.emailAddress);
    li.append("," + " " + contact.physAddress);
...
function listWorkContacts(workAddressBookToDisplay) {
  console.log("Hola");
  let workDiv = document.querySelector("div#work");
  workDiv.innerText = null;
  const ul = document.createElement("ul");
  Object.keys(workAddressBookToDisplay.contacts).forEach(function(key) {
    const contact = workAddressBookToDisplay.findContact(key);
    const li = document.createElement("li");
    li.append(contact.fullName());
    li.append("," + " " + contact.emailAddressTwo);
    li.append("," + " " + contact.physAddressTwo);
    li.setAttribute("id", contact.id);
    ul.append(li);
  });
  workDiv.append(ul);
...
function handleFormSubmission(event) {
  ...
  workAddressBook.addContact(newContact);
  listContacts(addressBook);
  listWorkContacts(workAddressBook);
}
